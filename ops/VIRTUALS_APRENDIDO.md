...
23. **Un trabajador que nunca ha servido un trabajo no está probado, aunque lleve semanas «arriba».** Leyendo la CLI 1.0.34 y `acp-node-v2` antes del primer trabajo real salieron tres fallos que lo habrían atascado en silencio: (a) `acp job list` da el estado en `jobStatus` con palabras v2 (`open`, `budget_set`, `funded`, `submitted`, `completed`), no en `phase` con REQUEST/TRANSACTION; (b) `acp job history` exige `--chain-id`; (c) en v2 el formulario del comprador es un **mensaje** `{"kind":"message","contentType":"requirement","content":"<json>"}`, no una clave `requirement`. Además, la `description` del trabajo es el nombre de la oferta (sirve para cobrar su precio), y `set-budget`/`submit` sólo se marcan como hechos si la CLI contesta. **Y ojo: con `--json` los errores salen por stdout como `{"error": …}` con código 1**; quien lea «la última línea JSON» sin mirar el código de salida da por entregado un `submit` fallido. Lo cubre `tests/test_acp_first_job_does_not_stall.py`, con las formas copiadas del código de la CLI.

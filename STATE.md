...
| Canales (26-ago) | ✅ **Bluesky `@pedibot.bsky.social` conectado y publicando** (primera publicación real verificada: guía de golpe de calor EN+ES). **Sin canal de Telegram** (decisión operador): en su lugar, avisos MUY puntuales a quien haya usado el bot — `pedibot announce "…"` con guarda de 14 días, línea de baja y `/stop`; la audiencia son chat ids en `tg_users` (documentado en /legal). X sigue manual (`publish/queue/x/`). Bloque «Dónde está PediBot» en la home + enlaces en pie y compositor. Verificación de Google Search Console servida en `/google73213559d998fa6d.html`. | `publish/social.py`, `announce.py` |

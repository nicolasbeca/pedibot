---
title: "¿Qué es la tos ferina y cómo cuidar a mi hijo?"
description: "Guía práctica para padres sobre la tos ferina, sus síntomas, cuidados en casa y cuándo buscar ayuda médica."
lang: es
topic: whooping_cough
date: 2026-09-02
prompt_version: article_v1
model: deepseek-v4-flash
sources:
  - "[1] MedlinePlus — \"Whooping Cough\" (2024), sección \"What is whooping cough?\", pág. 1 — https://medlineplus.gov/whoopingcough.html"
  - "[2] NHS — \"Whooping cough\" (2025), sección \"The whooping cough vaccine\", pág. 1 — https://www.nhs.uk/conditions/whooping-cough/"
  - "[3] NHS — \"Whooping cough\" (2025), sección \"Whooping cough\", pág. 1 — https://www.nhs.uk/conditions/whooping-cough/"
  - "[4] MedlinePlus — \"Whooping Cough\" (2024), sección \"Can whooping cough be prevented?\", pág. 1 — https://medlineplus.gov/whoopingcough.html"
  - "[5] CDC — \"About Whooping Cough\", sección \"What it is\", pág. 1 — https://www.cdc.gov/pertussis/about/index.html"
  - "[6] MedlinePlus — \"Whooping Cough\" (2024), sección \"What are the symptoms of whooping cough?\", pág. 1 — https://medlineplus.gov/whoopingcough.html"
  - "[7] MedlinePlus — \"Whooping Cough\" (2024), sección \"What causes whooping cough?\", pág. 1 — https://medlineplus.gov/whoopingcough.html"
  - "[8] CDC — \"About Whooping Cough\", sección \"About Whooping Cough Key points\", pág. 1 — https://www.cdc.gov/pertussis/about/index.html"
  - "[9] NHS — \"Whooping cough\" (2025), sección \"Whooping cough can be dangerous\", pág. 1 — https://www.nhs.uk/conditions/whooping-cough/"
  - "[10] CDC — \"About Whooping Cough\", sección \"People can unknowingly spread the bacteria\", pág. 1 — https://www.cdc.gov/pertussis/about/index.html"
draft: false
---

## Qué es
La tos ferina, también llamada pertussis, es una infección respiratoria muy contagiosa causada por una bacteria llamada Bordetella pertussis [5]. Esta bacteria se adhiere a las vías respiratorias y libera toxinas que dañan los tejidos y causan inflamación [5]. Se propaga de persona a persona al toser, estornudar o respirar muy cerca de alguien [7]. También puede contagiarse al tocar una superficie infectada y luego tocarse la nariz o la boca [7].

Los primeros síntomas son similares a un resfriado: moqueo, dolor de garganta y tos ocasional [3][6]. La fiebre alta es poco común [3]. Después de aproximadamente una semana, aparecen los accesos de tos que pueden durar varios minutos y empeoran por la noche [3]. Entre los síntomas típicos se incluyen:
- Un sonido "whoop" o silbido al respirar entre toses (aunque los bebés menores de 3 meses y algunos adultos no lo presentan) [3].
- Dificultad para respirar después de un acceso de tos, con posible coloración azul o gris en la piel [3].
- Producción de moco espeso que puede provocar vómitos [3].
- Enrojecimiento facial intenso [3].
- Agotamiento después de los accesos de tos [6].

Los síntomas suelen aparecer entre 5 y 10 días después de la exposición, aunque pueden tardar hasta 3 semanas [6]. La tos puede durar varias semanas o incluso meses [3][6]. En los bebés, los síntomas pueden ser diferentes: pueden toser poco o nada, pero presentar pausas en la respiración (apnea) y volverse azules [6]. Esto requiere atención médica inmediata [6].

La tos ferina es especialmente grave en bebés menores de 1 año que no han sido vacunados [1]. Aproximadamente la mitad de los bebés menores de 1 año que contraen la enfermedad necesitan hospitalización [1]. Los bebés menores de 6 meses tienen mayor riesgo de deshidratación, dificultades respiratorias, neumonía y convulsiones [9]. En niños mayores y adultos, la enfermedad suele ser menos grave, pero puede causar costillas doloridas, hernias, infecciones de oído o pérdida de orina al toser [9].

## Qué puedes hacer en casa
La mejor prevención es la vacunación [4]. La vacuna se administra como parte de la vacuna 6 en 1 a los 2, 4, 6 y 18 meses, y como refuerzo preescolar a los 3 años y 4 meses [2]. Si está embarazada, debe recibir la vacuna alrededor de la semana 20 para proteger al bebé durante sus primeras semanas de vida [2].

Para prevenir la propagación:
- Lávese las manos frecuentemente con agua y jabón [4].
- Evite tocarse la cara con las manos sin lavar [4].
- Limpie y desinfecte superficies que se tocan con frecuencia, incluyendo juguetes [4].
- Cúbrase la boca al toser o estornudar con un pañuelo o la manga, no con las manos [4].
- Quédese en casa cuando esté enfermo [4].
- Evite el contacto cercano con personas enfermas [4].
- Mantenga a los bebés y personas de alto riesgo alejados de quienes tienen tos ferina [4].

Si su hijo tiene tos ferina, los antibióticos pueden hacer la infección menos grave si se administran temprano [8]. También pueden acortar el tiempo de contagio [7]. Los miembros de la familia expuestos pueden recibir antibióticos para prevenir la enfermedad [4]. Consulte siempre a un profesional de salud para el tratamiento adecuado.

## Cuándo acudir al médico o a urgencias
Busque atención médica de inmediato si su bebé:
- Presenta pausas en la respiración o se pone azul [6].
- Tiene dificultad para respirar después de un acceso de tos [3].
- Se pone azul o gris [3].

También acuda al médico si:
- Su hijo tiene menos de 6 meses y presenta síntomas de tos ferina [9].
- Los accesos de tos son intensos, duran varios minutos o empeoran por la noche [3].
- Su hijo vomita después de toser o muestra signos de deshidratación [3][9].
- Tiene fiebre o cualquier signo de complicaciones como neumonía [9].

## Preguntas frecuentes
**¿Cuánto tiempo es contagiosa la tos ferina?**
Una persona es contagiosa aproximadamente 2 semanas después de comenzar a toser [7]. Los antibióticos pueden acortar este período [7].

**¿Puede alguien contagiar sin saber que tiene la enfermedad?**
Sí. Algunas personas tienen síntomas leves y no saben que tienen tos ferina, pero pueden transmitir la bacteria [10]. Muchos bebés se infectan por hermanos mayores, padres o cuidadores que no saben que la tienen [10].

**¿La vacuna protege completamente?**
La vacuna es la mejor forma de prevención [4]. Sin embargo, los síntomas pueden variar según la edad y si la persona fue vacunada o no [5]. Aun así, es fundamental completar todas las dosis recomendadas [2].

## Fuentes
[1] MedlinePlus — "Whooping Cough" (2024), section "What is whooping cough?", p. 1 — https://medlineplus.gov/whoopingcough.html
[2] NHS — "Whooping cough" (2025), section "The whooping cough vaccine", p. 1 — https://www.nhs.uk/conditions/whooping-cough/
[3] NHS — "Whooping cough" (2025), section "Whooping cough", p. 1 — https://www.nhs.uk/conditions/whooping-cough/
[4] MedlinePlus — "Whooping Cough" (2024), section "Can whooping cough be prevented?", p. 1 — https://medlineplus.gov/whoopingcough.html
[5] CDC — "About Whooping Cough", section "What it is", p. 1 — https://www.cdc.gov/pertussis/about/index.html
[6] MedlinePlus — "Whooping Cough" (2024), section "What are the symptoms of whooping cough?", p. 1 — https://medlineplus.gov/whoopingcough.html
[7] MedlinePlus — "Whooping Cough" (2024), section "What causes whooping cough?", p. 1 — https://medlineplus.gov/whoopingcough.html
[8] CDC — "About Whooping Cough", section "About Whooping Cough Key points", p. 1 — https://www.cdc.gov/pertussis/about/index.html
[9] NHS — "Whooping cough" (2025), section "Whooping cough can be dangerous", p. 1 — https://www.nhs.uk/conditions/whooping-cough/
[10] CDC — "About Whooping Cough", section "People can unknowingly spread the bacteria", p. 1 — https://www.cdc.gov/pertussis/about/index.html

*Esta guía resume guías pediátricas publicadas. No es consejo médico y no sustituye a tu pediatra. En una emergencia, llama a tu número de emergencias.*

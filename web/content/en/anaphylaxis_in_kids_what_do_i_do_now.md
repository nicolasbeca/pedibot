---
title: "Anaphylaxis in kids: What do I do now?"
description: "Anaphylaxis is a severe, fast allergic reaction. Act now: use the adrenaline injector and call emergency services."
lang: en
topic: anafilaxia
date: 2026-09-02
prompt_version: article_v1
model: deepseek-v4-flash
sources:
  - "[1] SEUP — \"Anafilaxia. Información para padres\" (2025), section \"¿QUÉ ES lA ANAFILAXIA?\", p. 1"
  - "[2] SEUP — \"Anafilaxia. Información para padres\" (2025), section \"HABONES\", p. 2"
draft: false
---

## What it is
Anaphylaxis is an extremely serious allergic reaction that affects several organs at once. It starts within minutes of contact with an allergen, such as food, medication, insect stings, or latex [1]. It is rare, affecting about 1% of the population [1].

Symptoms develop quickly and can include any combination of the following [1]:
- Itchy skin and scalp
- Swelling of the eyelids, lips, or ears
- Widespread intense redness
- Hives (welts)
- Trouble breathing
- Red eyes
- Sneezing
- Dizziness
- Loss of consciousness
- Vomiting

## What you can do at home
If you suspect anaphylaxis, act immediately [1]:
- Stay calm.
- Call emergency services (112 in Spain) or take your child to the nearest emergency department.
- If your child has a prescribed adrenaline auto-injector, use it right away [2].

How to use an adrenaline auto-injector [1]:
- Hold the injector with your dominant hand (the hand you write with).
- With the other hand, remove the safety cap.
- Press the injector firmly against the outer part of the thigh at a 90-degree angle.
- Press until you hear a "click" or press the button, depending on the device.
- Hold it in place for 10 seconds.
- Remove the injector. The needle will retract automatically.

If symptoms do not improve, a second auto-injector can be used 5 to 15 minutes after the first one [1].

While waiting for help [2]:
- Place your child in a comfortable position, preferably lying down with legs elevated.
- Avoid further contact with the triggering allergen if possible.
- If your child stops breathing or has no pulse, start CPR.

Do not offer any special food. If the cause was a food, take measures to avoid any contact or exposure [2].

## When to see a doctor or go to the emergency department
Anaphylaxis always requires urgent medical care. Call emergency services or go to the nearest emergency department immediately [1]. Without treatment, anaphylaxis can progress to shock or cardiovascular collapse [2].

After being discharged, watch for these signs that your child is getting worse and seek medical help [2]:
- Difficulty breathing or noisy breathing
- Drooling or blue-colored lips
- Pale skin
- Loss of consciousness

Your child can return to daycare or school once they have a good general condition and symptoms have disappeared [2].

## Common questions
**Can I use a second adrenaline injector?**
Yes. If symptoms do not improve, you can use a second auto-injector 5 to 15 minutes after the first one [1].

**What if the cause was a food?**
If a food caused the reaction, take measures to avoid contact or exposure to that food [2].

**How can I prevent anaphylaxis?**
If you know the allergen source, avoid contact or exposure to it [2].

## Sources
[1] SEUP — "Anafilaxia. Información para padres" (2025), section "¿QUÉ ES lA ANAFILAXIA?", p. 1
[2] SEUP — "Anafilaxia. Información para padres" (2025), section "HABONES", p. 2

*This guide summarises published paediatric guidelines. It is not medical advice and does not replace your paediatrician. In an emergency, call your local emergency number.*
